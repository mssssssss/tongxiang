...
public partial class otherSubject : System.Web.UI.Page
{
    ...
    public void _databind()
    {
        ...
        {
            //如果登录 就查找注册时城市的不是这个专业的话题
            com = new SqlCommand("select * from [User] left join Topic on Topic.AuthorId=[User].UserId left join [Major] on Topic.MajorId = [Major].MajorId where [User].ProvinceId=" + Session["provinceid"] + "and Topic.MajorId !=" + Session["majorid"] + "order by Topic.TopicTime desc", connection);
        }
        else
        {
            //如果未登录 就查找此城市的话题
            com = new SqlCommand("select * from [User] left join Topic on Topic.AuthorId=[User].UserId left join [Major] on Topic.MajorId = [Major].MajorId where [User].ProvinceId=" + Session["provinceid"] + "order by Topic.TopicTime desc", connection);
        }
