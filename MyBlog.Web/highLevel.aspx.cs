...
public partial class highLevel : System.Web.UI.Page
{
    ...
    public void _databind()
    {
        ...
        {
            //如果登录 就查找注册时城市的用户专业的3级话题
            com = new SqlCommand("select * from [User] left join Topic on Topic.AuthorId=[User].UserId left join [Major] on Topic.MajorId = [Major].MajorId where [User].ProvinceId=" + Session["provinceid"] + "and Topic.MajorId=" + Session["majorid"] + "and Topic.DiffLevel=3" + "order by Topic.TopicTime desc", connection);
        }
        else
        {
            //如果未登录 就查找此城市的3级话题
            com = new SqlCommand("select * from [User] left join Topic on Topic.AuthorId=[User].UserId left join [Major] on Topic.MajorId = [Major].MajorId where [User].ProvinceId=" + Session["provinceid"] + "and Topic.DiffLevel=3" + "order by Topic.TopicTime desc", connection);
        }
