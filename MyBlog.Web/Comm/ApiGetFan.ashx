<%@ WebHandler Language="C#" Class="ApiGetFan" %>
...
public class ApiGetFan : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        List<User> userList = fanService.getMyFan(int.Parse(authorId),int.Parse(page), int.Parse(limit));
        //注意返回的count是所有的数据，而不是分页当前页面的数据
        int counts = fanService.getMyFanCount(int.Parse(authorId));
